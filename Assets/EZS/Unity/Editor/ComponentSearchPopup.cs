...
namespace Wargon.ezs.Unity
{
    ...
    {
        private MonoEntity[] manyEntities;
        private SearchField addComponentsField;
        private string[] componentList;
        private string searchComponent;
        private Action<string, MonoEntity> OnAddComponent;
        private Vector2 scrollPos;
        private  GUIStyle buttonStyle;
        private Texture2D icon;
        private GUIStyle labelStyle;
        private int entitiesCount;
        public ComponentSearchPopup(Action<string, MonoEntity> action, MonoEntity[] entitieses, int count)
        {
        ...
        private void DrawComponentBtn(string name)
        {
            ...
            if (GUILayout.Button(content,buttonStyle))
            {
                for (var i = 0; i < entitiesCount; i++)
                    OnAddComponent?.Invoke(name, manyEntities[i]);
